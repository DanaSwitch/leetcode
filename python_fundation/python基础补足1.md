...
| ------------ | --------------------- | -------------------- | ----------------------- | ---------------------- |
| **定义方式** | `[a, b]` (方括号)     | `(a, b)` (圆括号)    | `{a, b}` (花括号)       | `{k: v}` (键值对)      |
| **有序性**   | **有序** (支持索引)   | **有序** (支持索引)  | **无序** (不支持索引)   | **有序** (3.7+版本)    |
| **可变性**   | **可变** (可增删改)   | **不可变**           | **可变** (可增删)       | **可变** (键不可改)    |
| **元素重复** | 允许重复              | 允许重复             | **不允许重复**          | **键(Key)唯一**        |
| **查找速度** | 慢 ( $O(n)$ )         | 慢 ( $O(n)$ )        | **极快** ( $O(1)$ )     | **极快** (根据键查找)  |
| **典型场景** | 存储顺序数据、栈/队列 | 函数返回值、配置常量 | **去重**、交集/并集运算 | **快速索引**、映射关系 |
...
异常有很多种（比如 `IndexError` 是索引越界，`TypeError` 是类型不对）。

- `int()` 函数被设计为：当输入类型对（是字符串）但**内容长得不像数字**时，专门抛出 `ValueError`。
- 写 `except ValueError` 的意思是：“我只处理**格式不对**这种错误。如果是其他莫名其妙的错误（比如内存不足），我不拦着，让程序崩给我看。”——这是一种精准的防御。
...
简单来说：**`join` 是“胶水”，用来把一个列表里的字符串粘成一个长字符串**

- **`" "` (前面这个字符串)**：这是**连接符（分隔符）**。这里是一个空格。意思是：“我要用**空格**把后面列表里的东西连起来”。
- **`.join(...)`**：这是方法名。
- **`(res_strings)`**：这是括号里的参数，必须是一个**全是字符串**的列表（或者元组）。

### 直观演示

假设 `res_strings` 里的内容是： `['0-1', '3-4', '6-7']`
